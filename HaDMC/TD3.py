...
class Actor(nn.Module):
    def __init__(self, state_dim, discrete_action_dim, parameter_action_dim, max_action):
        ...
        self.l3_1 = nn.Linear(hidden_dim, discrete_action_dim)
        self.l3_2 = nn.Linear(hidden_dim, parameter_action_dim)
...
class Critic(nn.Module):
    def __init__(self, state_dim, discrete_action_dim, parameter_action_dim):
        ...
        # Q1 architecture
        self.l1 = nn.Linear(state_dim + discrete_action_dim + parameter_action_dim, hidden_dim)
        self.l2 = nn.Linear(hidden_dim, hidden_dim)
        self.l3 = nn.Linear(hidden_dim, 1)

        # Q2 architecture
        self.l4 = nn.Linear(state_dim + discrete_action_dim + parameter_action_dim, hidden_dim)
...
class TD3(object):
    def __init__(
        ...
        self.lr_actor = 3.5e-5
        self.lr_critic =      self.lr_actor
        self.actor_optimizer = torch.optim.AdamW(self.actor.parameters(), lr=self.lr_actor)
        print("learning rate = ", self.lr_actor)
    ...
    def train(self, replay_buffer, batch_size=128):
        ...
        training_type = 1
        state, discrete_emb, parameter_emb, next_state, reward, not_done = replay_buffer.sample(
            batch_size, training_type)
        with torch.no_grad():
